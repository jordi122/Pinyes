...
SplitView {
    ...
    function omplirModels() {
        list_viewpeutresComp.model = crosses;     afegirNomsaLlistesOrd("Crosses","colNom");
        list_viewpeutresComp.model = primeres;    afegirNomsaLlistesOrd("Primeres","colNom");
        list_viewpeutresComp.model = baixos;      afegirNomsaLlistesOrd("Baixos","colNom");
        list_viewpeutresComp.model = agulles;     afegirNomsaLlistesOrd("Agulles","colNom");
        list_viewpeutresComp.model = contraforts; afegirNomsaLlistesOrd("Contraforts","colNom");
        list_viewpeutresComp.model = pinya;       afegirNomsaLlistesOrd("Pinya","colNom");
        embolcallcbItems.currentIndex = intNomPosicio;
        switch (intNomPosicio) {
        case 0:
            list_viewpeutresComp.model = pinya;
            break;
        case 1:
            list_viewpeutresComp.model = baixos;
            break;
        case 2:
            list_viewpeutresComp.model = crosses;
            break;
        case 3:
            list_viewpeutresComp.model = agulles;
            break;
        case 4:
            list_viewpeutresComp.model = contraforts;
            break;
        case 5:
            list_viewpeutresComp.model = primeres;
            break;
        default:
            list_viewpeutresComp.model = pinya;
            break;
        ...
        db.transaction(
                    function(tx) {
                        ...
                        for(var j = 0; j < llista.rows.length ; j++) {
                            for(var i = 0; i < list_viewpeutresComp.model.count ; i++) {
                                if (list_viewpeutresComp.model.get(i).colNom === llista.rows.item(j).colNom) {
                                    list_viewpeutresComp.model.remove(i);
                                    break;
                                }
                            }
                        }
    ...
    function afegirNomsaLlistesOrd(posicio,ordreDes) {
        var sentit = "ASC";
        if (list_viewpeutresComp.sortIndicatorOrder===1) {
            sentit = "DESC"
        }
        db.transaction(
                    function(tx) {
                        ...
                        list_viewpeutresComp.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            ...
                            if (llista2.rows.length ===0) {
                                list_viewpeutresComp.model.append({"colNom": llista.rows.item(i).colNom,"espatlla": llista.rows.item(i).espatlla,"mans": llista.rows.item(i).mans})
    ...
    function afegirPinyesaLlistesLocal(tipusPinya) {
        db.transaction(
                    function(tx) {
                        ...
                        list_viewpeutres.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            list_viewpeutres.model.append({"colNom": llista.rows.item(i).colNom,"data": llista.rows.item(i).data})
                        }
                    }
                    )
    }
            ...
            GroupBox {
                ...
                GridLayout {
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            afegirPinyesaLlistesLocal("llistapeutres");
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            list_viewpeutres.model.remove(indexTaula2)
                            AccionsBD.esborrarPinya("llistapeutres",pinyaActual)
                            indexTaula2=-1
                    ...
                    TableView {
                        ...
                        Component.onCompleted: {
                            list_viewpeutres.model = llistapeutres;
                            afegirPinyesaLlistesLocal("llistapeutres");
                        }
                        itemDelegate: {
                            return itemDelegateCastells
                    ...
                    ToolButton  {
                        ...
                        onClicked: {
                            AccionsBD.buscarRegistreaTaula(text_input1.text,"llistapeutres");
                            ...
                            if (text_input1.text==="Nom..." || !xequeig) {
                                ...
                                AccionsBD.backupPinya(text_input1.text,taulaActual,"llistapeutres");
                                afegirPinyesaLlistesLocal("llistapeutres");
                                barra.color = "green";
