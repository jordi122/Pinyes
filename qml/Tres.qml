...
SplitView {
    ...
    function omplirModels() {
        list_viewtresComp.model = crosses;     afegirNomsaLlistesOrd("Crosses","colNom");
        list_viewtresComp.model = primeres;    afegirNomsaLlistesOrd("Primeres","colNom");
        list_viewtresComp.model = baixos;      afegirNomsaLlistesOrd("Baixos","colNom");
        list_viewtresComp.model = agulles;     afegirNomsaLlistesOrd("Agulles","colNom");
        list_viewtresComp.model = contraforts; afegirNomsaLlistesOrd("Contraforts","colNom");
        list_viewtresComp.model = pinya;       afegirNomsaLlistesOrd("Pinya","colNom");
        embolcallcbItems.currentIndex = intNomPosicio;
        switch (intNomPosicio) {
        case 0:
            list_viewtresComp.model = pinya;
            break;
        case 1:
            list_viewtresComp.model = baixos;
            break;
        case 2:
            list_viewtresComp.model = crosses;
            break;
        case 3:
            list_viewtresComp.model = agulles;
            break;
        case 4:
            list_viewtresComp.model = contraforts;
            break;
        case 5:
            list_viewtresComp.model = primeres;
            break;
        default:
            list_viewtresComp.model = pinya;
            break;
        ...
        db.transaction(
                    function(tx) {
                        ...
                        for(var j = 0; j < llista.rows.length ; j++) {
                            for(var i = 0; i < list_viewtresComp.model.count ; i++) {
                                if (list_viewtresComp.model.get(i).colNom === llista.rows.item(j).colNom) {
                                    list_viewtresComp.model.remove(i);
                                    break;
                                }
                            }
                        }
    ...
    function afegirNomsaLlistesOrd(posicio,ordreDes) {
        var sentit = "ASC";
        if (list_viewtresComp.sortIndicatorOrder===1) {
            sentit = "DESC"
        }
        db.transaction(
                    function(tx) {
                        ...
                        for(var i = 0; i < llista.rows.length; i++) {
                            ...
                            if (llista2.rows.length ===0) {
                                list_viewtresComp.model.append({"colNom": llista.rows.item(i).colNom,"espatlla": llista.rows.item(i).espatlla,"mans": llista.rows.item(i).mans})
    ...
    function afegirPinyesaLlistesLocal(tipusPinya) {
        db.transaction(
                    function(tx) {
                        ...
                        list_viewtres.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            list_viewtres.model.append({"colNom": llista.rows.item(i).colNom,"data": llista.rows.item(i).data})
                        }
                    }
                    )
    }
            ...
            GroupBox {
                ...
                GridLayout {
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            afegirPinyesaLlistesLocal("llistatres");
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            list_viewtres.model.remove(indexTaula2)
                            AccionsBD.esborrarPinya("llistatres",pinyaActual)
                            indexTaula2=-1
                    ...
                    TableView {
                        ...
                        Component.onCompleted: {
                            list_viewtres.model = llistatres;
                            afegirPinyesaLlistesLocal("llistatres");
                        }
                        itemDelegate: {
                            return itemDelegateCastells
                    ...
                    ToolButton  {
                        ...
                        onClicked: {
                            AccionsBD.buscarRegistreaTaula(text_input1.text,"llistatres");
                            ...
                            if (text_input1.text==="Nom..." || !xequeig) {
                                ...
                                AccionsBD.backupPinya(text_input1.text,taulaActual,"llistatres");
                                afegirPinyesaLlistesLocal("llistatres");
                                barra.color = "green";
