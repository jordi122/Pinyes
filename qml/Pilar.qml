...
SplitView {
    ...
    function omplirModels() {
        list_viewpilarComp.model = crosses;     afegirNomsaLlistesOrd("Crosses","colNom");
        list_viewpilarComp.model = primeres;    afegirNomsaLlistesOrd("Primeres","colNom");
        list_viewpilarComp.model = baixos;      afegirNomsaLlistesOrd("Baixos","colNom");
        list_viewpilarComp.model = agulles;     afegirNomsaLlistesOrd("Agulles","colNom");
        list_viewpilarComp.model = contraforts; afegirNomsaLlistesOrd("Contraforts","colNom");
        list_viewpilarComp.model = pinya;       afegirNomsaLlistesOrd("Pinya","colNom");
        embolcallcbItems.currentIndex = intNomPosicio;
        switch (intNomPosicio) {
        case 0:
            list_viewpilarComp.model = pinya;
            break;
        case 1:
            list_viewpilarComp.model = baixos;
            break;
        case 2:
            list_viewpilarComp.model = crosses;
            break;
        case 3:
            list_viewpilarComp.model = agulles;
            break;
        case 4:
            list_viewpilarComp.model = contraforts;
            break;
        case 5:
            list_viewpilarComp.model = primeres;
            break;
        default:
            list_viewpilarComp.model = pinya;
            break;
        ...
        db.transaction(
                    function(tx) {
                        ...
                        for(var j = 0; j < llista.rows.length ; j++) {
                            for(var i = 0; i < list_viewpilarComp.model.count ; i++) {
                                if (list_viewpilarComp.model.get(i).colNom === llista.rows.item(j).colNom) {
                                    list_viewpilarComp.model.remove(i);
                                    break;
                                }
                            }
                        }
    ...
    function afegirNomsaLlistesOrd(posicio,ordreDes) {
        var sentit = "ASC";
        if (list_viewpilarComp.sortIndicatorOrder===1) {
            sentit = "DESC"
        }
        db.transaction(
                    function(tx) {
                        ...
                        list_viewpilarComp.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            ...
                            if (llista2.rows.length ===0) {
                                list_viewpilarComp.model.append({"colNom": llista.rows.item(i).colNom,"espatlla": llista.rows.item(i).espatlla,"mans": llista.rows.item(i).mans})
    ...
    function afegirPinyesaLlistesLocal(tipusPinya) {
        db.transaction(
                    function(tx) {
                        ...
                        list_viewpilar.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            list_viewpilar.model.append({"colNom": llista.rows.item(i).colNom,"data": llista.rows.item(i).data})
                        }
                    }
                    )
    }
            ...
            GroupBox {
                ...
                GridLayout {
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            afegirPinyesaLlistesLocal("llistapilar");
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            list_viewpilar.model.remove(indexTaula2)
                            AccionsBD.esborrarPinya("llistapilar",pinyaActual)
                            indexTaula2=-1
                    ...
                    TableView {
                        ...
                        Component.onCompleted: {
                            list_viewpilar.model = llistapilar;
                            afegirPinyesaLlistesLocal("llistapilar");
                        }
                        itemDelegate: {
                            return itemDelegateCastells
                    ...
                    ToolButton  {
                        ...
                        onClicked: {
                            AccionsBD.buscarRegistreaTaula(text_input1.text,"llistapilar");
                            ...
                            if (text_input1.text==="Nom..." || !xequeig) {
                                ...
                                AccionsBD.backupPinya(text_input1.text,taulaActual,"llistapilar");
                                afegirPinyesaLlistesLocal("llistapilar");
                                barra.color = "green";
