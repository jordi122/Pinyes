...
SplitView {
    ...
    function omplirModels() {
        list_viewsetComp.model = crosses;     afegirNomsaLlistesOrd("Crosses","colNom");
        list_viewsetComp.model = primeres;    afegirNomsaLlistesOrd("Primeres","colNom");
        list_viewsetComp.model = baixos;      afegirNomsaLlistesOrd("Baixos","colNom");
        list_viewsetComp.model = agulles;     afegirNomsaLlistesOrd("Agulles","colNom");
        list_viewsetComp.model = contraforts; afegirNomsaLlistesOrd("Contraforts","colNom");
        list_viewsetComp.model = pinya;       afegirNomsaLlistesOrd("Pinya","colNom");
        embolcallcbItems.currentIndex = intNomPosicio;
        switch (intNomPosicio) {
        case 0:
            list_viewsetComp.model = pinya;
            break;
        case 1:
            list_viewsetComp.model = baixos;
            break;
        case 2:
            list_viewsetComp.model = crosses;
            break;
        case 3:
            list_viewsetComp.model = agulles;
            break;
        case 4:
            list_viewsetComp.model = contraforts;
            break;
        case 5:
            list_viewsetComp.model = primeres;
            break;
        default:
            list_viewsetComp.model = pinya;
            break;
        ...
        db.transaction(
                    function(tx) {
                        ...
                        for(var j = 0; j < llista.rows.length ; j++) {
                            for(var i = 0; i < list_viewsetComp.model.count ; i++) {
                                if (list_viewsetComp.model.get(i).colNom === llista.rows.item(j).colNom) {
                                    list_viewsetComp.model.remove(i);
                                    break;
                                }
                            }
                        }
    ...
    function afegirNomsaLlistesOrd(posicio,ordreDes) {
        var sentit = "ASC";
        if (list_viewsetComp.sortIndicatorOrder===1) {
            sentit = "DESC"
        }
        db.transaction(
                    function(tx) {
                        ...
                        for(var i = 0; i < llista.rows.length; i++) {
                            ...
                            if (llista2.rows.length ===0) {
                                list_viewsetComp.model.append({"colNom": llista.rows.item(i).colNom,"espatlla": llista.rows.item(i).espatlla,"mans": llista.rows.item(i).mans})
    ...
    function afegirPinyesaLlistesLocal(tipusPinya) {
        db.transaction(
                    function(tx) {
                        ...
                        list_viewset.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            list_viewset.model.append({"colNom": llista.rows.item(i).colNom,"data": llista.rows.item(i).data})
                        }
                    }
                    )
    }
            ...
            GroupBox {
                ...
                GridLayout {
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            afegirPinyesaLlistesLocal("llistaset");
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            list_viewset.model.remove(indexTaula2)
                            AccionsBD.esborrarPinya("llistaset",pinyaActual)
                            indexTaula2=-1
                    ...
                    ToolButton  {
                        ...
                        onClicked: {
                            AccionsBD.buscarRegistreaTaula(text_input1.text,"llistaset");
                            ...
                            if (text_input1.text==="Nom..." || !xequeig) {
                                ...
                                AccionsBD.backupPinya(text_input1.text,taulaActual,"llistaset");
                                afegirPinyesaLlistesLocal("llistaset");
                                barra.color = "green";
