...
SplitView {
    ...
    function omplirModels() {
        list_viewquatreaComp.model = crosses;     afegirNomsaLlistesOrd("Crosses","colNom");
        list_viewquatreaComp.model = primeres;    afegirNomsaLlistesOrd("Primeres","colNom");
        list_viewquatreaComp.model = baixos;      afegirNomsaLlistesOrd("Baixos","colNom");
        list_viewquatreaComp.model = agulles;     afegirNomsaLlistesOrd("Agulles","colNom");
        list_viewquatreaComp.model = contraforts; afegirNomsaLlistesOrd("Contraforts","colNom");
        list_viewquatreaComp.model = pinya;       afegirNomsaLlistesOrd("Pinya","colNom");
        embolcallcbItems.currentIndex = intNomPosicio;
        switch (intNomPosicio) {
        case 0:
            list_viewquatreaComp.model = pinya;
            break;
        case 1:
            list_viewquatreaComp.model = baixos;
            break;
        case 2:
            list_viewquatreaComp.model = crosses;
            break;
        case 3:
            list_viewquatreaComp.model = agulles;
            break;
        case 4:
            list_viewquatreaComp.model = contraforts;
            break;
        case 5:
            list_viewquatreaComp.model = primeres;
            break;
        default:
            list_viewquatreaComp.model = pinya;
            break;
        ...
        db.transaction(
                    function(tx) {
                        ...
                        for(var j = 0; j < llista.rows.length ; j++) {
                            for(var i = 0; i < list_viewquatreaComp.model.count ; i++) {
                                if (list_viewquatreaComp.model.get(i).colNom === llista.rows.item(j).colNom) {
                                    list_viewquatreaComp.model.remove(i);
                                    break;
                                }
                            }
                        }
    ...
    function afegirNomsaLlistesOrd(posicio,ordreDes) {
        var sentit = "ASC";
        if (list_viewquatreaComp.sortIndicatorOrder===1) {
            sentit = "DESC"
        }
        db.transaction(
                    function(tx) {
                        ...
                        list_viewquatreaComp.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            ...
                            if (llista2.rows.length ===0) {
                                list_viewquatreaComp.model.append({"colNom": llista.rows.item(i).colNom,"espatlla": llista.rows.item(i).espatlla,"mans": llista.rows.item(i).mans})
    ...
    function afegirPinyesaLlistesLocal(tipusPinya) {
        db.transaction(
                    function(tx) {
                        ...
                        list_viewquatrea.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            list_viewquatrea.model.append({"colNom": llista.rows.item(i).colNom,"data": llista.rows.item(i).data})
                        }
                    }
                    )
    }
            ...
            GroupBox {
                ...
                GridLayout {
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            afegirPinyesaLlistesLocal("llistaquatrea");
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            list_viewquatrea.model.remove(indexTaula2)
                            AccionsBD.esborrarPinya("llistaquatrea",pinyaActual)
                            indexTaula2=-1
                    ...
                    TableView {
                        ...
                        Component.onCompleted: {
                            list_viewquatrea.model = llistaquatrea;
                            afegirPinyesaLlistesLocal("llistaquatrea");
                        }
                        itemDelegate: {
                            return itemDelegateCastells
                    ...
                    ToolButton  {
                        ...
                        onClicked: {
                            AccionsBD.buscarRegistreaTaula(text_input1.text,"llistaquatrea");
                            ...
                            if (text_input1.text==="Nom..." || !xequeig) {
                                ...
                                AccionsBD.backupPinya(text_input1.text,taulaActual,"llistaquatrea");
                                afegirPinyesaLlistesLocal("llistaquatrea");
                                barra.color = "green";
