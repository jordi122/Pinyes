...
SplitView {
    ...
    function omplirModels() {
        list_viewcincComp.model = crosses;     afegirNomsaLlistesOrd("Crosses","colNom");
        list_viewcincComp.model = primeres;    afegirNomsaLlistesOrd("Primeres","colNom");
        list_viewcincComp.model = baixos;      afegirNomsaLlistesOrd("Baixos","colNom");
        list_viewcincComp.model = agulles;     afegirNomsaLlistesOrd("Agulles","colNom");
        list_viewcincComp.model = contraforts; afegirNomsaLlistesOrd("Contraforts","colNom");
        list_viewcincComp.model = pinya;       afegirNomsaLlistesOrd("Pinya","colNom");
        embolcallcbItems.currentIndex = intNomPosicio;
        switch (intNomPosicio) {
        case 0:
            list_viewcincComp.model = pinya;
            break;
        case 1:
            list_viewcincComp.model = baixos;
            break;
        case 2:
            list_viewcincComp.model = crosses;
            break;
        case 3:
            list_viewcincComp.model = agulles;
            break;
        case 4:
            list_viewcincComp.model = contraforts;
            break;
        case 5:
            list_viewcincComp.model = primeres;
            break;
        default:
            list_viewcincComp.model = pinya;
            break;
        ...
        db.transaction(
                    function(tx) {
                        ...
                        for(var j = 0; j < llista.rows.length ; j++) {
                            for(var i = 0; i < list_viewcincComp.model.count ; i++) {
                                if (list_viewcincComp.model.get(i).colNom === llista.rows.item(j).colNom) {
                                    list_viewcincComp.model.remove(i);
                                    break;
                                }
                            }
                        }
    ...
    function afegirNomsaLlistesOrd(posicio,ordreDes) {
        var sentit = "ASC";
        if (list_viewcincComp.sortIndicatorOrder===1) {
            sentit = "DESC"
        }
        db.transaction(
                    function(tx) {
                        ...
                        for(var i = 0; i < llista.rows.length; i++) {
                            ...
                            if (llista2.rows.length ===0) {
                                list_viewcincComp.model.append({"colNom": llista.rows.item(i).colNom,"espatlla": llista.rows.item(i).espatlla,"mans": llista.rows.item(i).mans})
    ...
    function afegirPinyesaLlistesLocal(tipusPinya) {
        db.transaction(
                    function(tx) {
                        ...
                        list_viewcinc.model.clear();
                        for(var i = 0; i < llista.rows.length; i++) {
                            list_viewcinc.model.append({"colNom": llista.rows.item(i).colNom,"data": llista.rows.item(i).data})
                        }
                    }
                    )
    }
            ...
            GroupBox {
                ...
                GridLayout {
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            afegirPinyesaLlistesLocal("llistacinc");
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            list_viewcinc.model.remove(indexTaula2)
                            AccionsBD.esborrarPinya("llistacinc",pinyaActual)
                            indexTaula2=-1
                    ...
                    TableView {
                        ...
                        Component.onCompleted: {
                            list_viewcinc.model = llistacinc;
                            afegirPinyesaLlistesLocal("llistacinc");
                        }
                        itemDelegate: {
                            return itemDelegateCastells
                    ...
                    ToolButton  {
                        ...
                        onClicked: {
                            AccionsBD.buscarRegistreaTaula(text_input1.text,"llistacinc");
                            ...
                            if (text_input1.text==="Nom..." || !xequeig) {
                                ...
                                AccionsBD.backupPinya(text_input1.text,taulaActual,"llistacinc");
                                afegirPinyesaLlistesLocal("llistacinc");
                                barra.color = "green";
